...
export const runPipeline = (task: string, scope: string) => {
  // Call local Next.js proxy (forwards to backend or n8n server-side).
  ...
  // Fire the pipeline request and await the final combined response
  ...
  fetch(webhookUrl, {
    ...
    .then(async (res) => {
      ...
      // Mark all agents as done when the pipeline finishes.
      // This prevents UI rows from getting stuck in "thinking..." state.
      for (const agent of agents) {
        onMessageCallback?.({
          agent,
          status: 'done',
          message: `${agent.charAt(0).toUpperCase() + agent.slice(1)} completed.`,
        });
      }
