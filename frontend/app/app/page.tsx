...
const ConfettiRain = ({ active }: { active: boolean }) => {
  ...
  useEffect(() => {
    if (active) {
      const colors = ['#D4FF33', '#E5FF7A', '#F3FFC2', '#557A1A'];
...
export default function AppPage() {
  ...
  const handleExportZip = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `maad-export-${Date.now()}.zip`;
  ...
  const examples = [
    'Single-file HTML landing page with embedded CSS and JS',
    'Simple calculator in pure HTML CSS JS (one index.html file)',
    'Portfolio hero page using pages/index.tsx and local components',
    'Student dashboard with auth',
        ...
        <AnimatePresence>
          {isDemoMode && (
            <motion.div
              ...
              className="w-full bg-[var(--accent)] text-[var(--ink-on-accent)] px-4 py-2 flex items-center justify-center border-b border-[var(--border)] z-30 text-center"
            >
        ...
        <AnimatePresence>
          {state.appStatus === 'done' && (
            <motion.div 
              ...
              className="w-full bg-[var(--accent)] text-[var(--ink-on-accent)] px-4 md:px-6 py-4 flex items-center justify-center border-b border-[var(--bg-primary)]"
            >
          ...
          {state.appStatus === 'error' && (
            <motion.div 
              initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              className="w-full bg-[var(--accent-dim)] text-[var(--ink-on-accent)] px-4 md:px-6 py-4 flex items-center justify-center border-b border-[var(--bg-primary)]"
            >
              <span className="font-display font-bold text-[14px] md:text-[15px] uppercase tracking-wider text-center">
            ...
            <div className="flex flex-col gap-6 mb-10 w-full">
              ...
              {state.appStatus === 'error' ? (
                <button
                  ...
                  className="w-full relative min-h-[56px] flex items-center justify-center bg-[var(--accent-dim)] text-[var(--ink-on-accent)] font-display font-bold text-[16px] md:text-[18px] uppercase tracking-[0.1em] hover:bg-[var(--accent)] transition-all duration-300 focus:outline-none focus-visible:ring-4 focus-visible:ring-[var(--accent)]/50"
                ...
                <button
                  ...
                  className={`w-full relative min-h-[56px] flex items-center justify-center text-[var(--ink-on-accent)] font-display font-bold text-[16px] md:text-[18px] uppercase tracking-[0.1em] transition-all duration-300 active:scale-[0.98] focus:outline-none focus-visible:ring-4 focus-visible:ring-[var(--accent)]/50 disabled:opacity-40 disabled:cursor-not-allowed ${
                    isDemoMode ? 'bg-[#E5FF7A] hover:bg-[#F2FFC0]' : 'bg-[var(--accent)] hover:bg-[var(--accent-dim)]'
                  }`}
                  style={{ borderRadius: 0 }}
                >
                  {state.appStatus === 'running' ? (
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center gap-1">
                        <motion.div className="w-1.5 h-1.5 bg-[var(--ink-on-accent)]" animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1, repeat: Infinity, delay: 0 }} />
                        <motion.div className="w-1.5 h-1.5 bg-[var(--ink-on-accent)]" animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1, repeat: Infinity, delay: 0.2 }} />
                        <motion.div className="w-1.5 h-1.5 bg-[var(--ink-on-accent)]" animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1, repeat: Infinity, delay: 0.4 }} />
                      </div>
                      WORKING...
                    </div>
                  ) : (
