...
const LogStream = ({ entries, isRunning }: LogStreamProps) => {
  ...
  const getTypeClasses = (type: LogEntryType) => {
    switch (type) {
      case 'success':
        return 'text-[#6B9E6B]';
      case 'error':
        return 'text-[#C0524A] bg-[#C0524A]/10 px-1';
      case 'code':
  ...
  return (
    <div className="w-full bg-[#0A0A0A] border border-[var(--border)] flex flex-col font-mono" style={{ borderRadius: 0 }}>
      {/* Header Row */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border)] bg-[#111111]">
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setAutoScroll(!autoScroll)}>
          ...
          <div className="w-6 h-3 bg-[#0A0A0A] border border-[var(--border)] relative flex items-center transition-colors px-[1px]">
            <motion.div 
