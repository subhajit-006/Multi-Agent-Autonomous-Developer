...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col justify-center items-center overflow-hidden bg-[#0A0A0A]">
      {/* ── Background Elements ─────────────────────────────────── */}
      ...
      <div className="container relative z-10 flex flex-col items-center text-center">
        ...
        >
          <Link href="/app">
            <button
              className="px-8 py-3.5 bg-[var(--accent)] text-[#0A0A0A] font-display font-medium text-[16px] transition-all hover:scale-[1.02] hover:bg-[#F59E0B]"
              style={{ borderRadius: 0 }}
            >
              Start Building →
            </button>
      ...
      {/* ── Bottom Stats Line ───────────────────────────────────── */}
      <div className="absolute bottom-12 left-0 w-full z-10 px-6 container">
        <motion.div
        ...
        <motion.div
          ...
          className="flex flex-wrap justify-center md:justify-between items-center gap-4 text-[13px] text-[var(--text-muted)]"
        >
          {['5 Agents', 'Real-time Pipeline', 'Auto-debugged', 'GitHub Push'].map((stat, i, arr) => (
            <React.Fragment key={stat}>
              <span className="font-body uppercase tracking-wider">{stat}</span>
              {i < arr.length - 1 && (
                <span className="hidden md:inline h-3 w-[1px] bg-[var(--border)]" />
              )}
            </React.Fragment>
          ))}
        </motion.div>
