...
const StatusIndicator = ({ status, summary }: { status: AgentStatus; summary?: string }) => {
  ...
  if (status === 'done') {
    return (
      <div className="flex items-center gap-2">
        <div className="w-2 h-2 rounded-full bg-[#6B9E6B]" />
        <span className="font-body text-[13px] text-[#6B9E6B]">
          {summary || 'completed'}
        </span>
      </div>
    );
  ...
  if (status === 'error') {
    return (
      <div className="flex items-center gap-2">
        <div className="w-2 h-2 rounded-full bg-[#D94E4E]" />
        <span className="font-body text-[13px] text-[#D94E4E]">error occurred</span>
      </div>
    );
  }
            ...
            <div key={agent.id} className="border-b border-[var(--border)]">
              ...
              >
                <div className="flex items-center gap-4">
                  {/* Icon */}
                  <div className="w-8 h-8 flex items-center justify-center border border-[var(--border)] bg-[#111111]">
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <div className="bg-[#050505] p-6 border-t border-[var(--border)] border-dashed font-mono text-[12px] text-[var(--text-secondary)] leading-relaxed max-h-[250px] overflow-y-auto">
                      {agent.logs.length === 0 ? (
